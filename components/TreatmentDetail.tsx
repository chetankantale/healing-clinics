...
export function TreatmentDetail({ slug }: { slug: string }) {
  const treatment = treatmentItems.find((item) => item.slug === slug);
  if (!treatment) {
    return (
      <div className="bg-slate-50 py-20">
        <div className="mx-auto max-w-4xl px-6 text-center text-slate-700">
          <p className="text-lg font-semibold">Treatment not found.</p>
        </div>
      </div>
    );
  }
